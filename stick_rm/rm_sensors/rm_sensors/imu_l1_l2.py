...
#####
        ...
        self.l3=dict({"r":400.0,"p":400.0,"y":400.0})
...
#        self.timer_ = self.create_timer(0.01, self.send)
    ...
    def publish_imu(self):
        try:
            ...
            #bevel
            self.l3["y"] = safe_float(z[8])
            self.l3["r"] = safe_float(z[9])
            self.l3["p"] = safe_float(z[10]) % 360
            ...
            print(f"L1 [Y:{self.l1['y']:.2f} R:{self.l1['r']:.2f} P:{self.l1['p']:.2f}] | "
                  f"L2 [Y:{self.l2['y']:.2f} R:{self.l2['r']:.2f} P:{self.l2['p']:.2f}] | "
                  f"L2 [Y:{self.l3['y']:.2f} R:{self.l3['r']:.2f} P:{self.l3['p']:.2f}] | ")
